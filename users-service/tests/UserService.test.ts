...
describe('UserService', () => {
    ...
    describe('findById', () => {
        ...
        it('should fetch from database and cache if not in cache', async () => {
            ...
            (UserRepository.prototype.findById as jest.Mock).mockResolvedValueOnce(mockUser);

            const result = await userService.findById(userId);

            expect(result).toEqual(mockUser);
            expect(redis.get).toHaveBeenCalledWith(`user:${userId}`);
            expect(redis.set).toHaveBeenCalledWith(`user:${userId}`, JSON.stringify(mockUser), 'EX', 60);
        });

        it('should throw error if user not found', async () => {
            const userId = '1';
            (redis.get as jest.Mock).mockResolvedValueOnce(null);
            (UserRepository.prototype.findById as jest.Mock).mockResolvedValueOnce(null);
    ...
    describe('update', () => {
        it('should update user and publish event', async () => {
            ...
            (UserRepository.prototype.findById as jest.Mock).mockResolvedValueOnce(mockUser);
            (UserRepository.prototype.updateUser as jest.Mock).mockResolvedValueOnce({ ...mockUser, ...updateData });
